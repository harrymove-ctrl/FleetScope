/**
 * Bundled snapshot for `/console` when the API is unreachable (static Cloud
 * Run web, empty CORS, or local Astro with no PUBLIC_API_BASE_URL).
 * Keep the session ids, revisions, and probe payloads in lockstep with
 * `apps/api/src/cloud/evidence.ts`.
 */
